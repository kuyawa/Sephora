 ...
 *
 * Github:  https://github.com/simonwaldherr/markdown.js/
 * Version: 0.3.4
 */
...
var markdown = {
  ...
  parse: function (str, strict) {
    ...
    if (strict !== true) {
      markdown.regexobject.lists = /^((\s*(\*|\d\.) [^\n]+)\n)+/gm;
    }
    ...
    while ((stra = markdown.regexobject.code.exec(str)) !== null) {
      crc32str = markdown.crc32(stra[0]);
      markdown.codeblocks[crc32str] = '<code>\n' + markdown.htmlEncode(stra[1]).replace(/\n/gm, '<br/>').replace(/\ /gm, '&nbsp;') + '</code>';
    ...
    while ((stra = markdown.regexobject.links.exec(str)) !== null) {
      if (stra[0].substr(0, 1) === '!') {
        ...
        str = str.replace(stra[0], '<a ' + markdown.mmdCSSclass(stra[2], strict) + 'href="' + stra[2] + '">' + stra[1] + '</a>');
      }
    }
    ...
    while ((stra = markdown.regexobject.url.exec(str)) !== null) {
      ...
      str = str.replace(stra[0], '<a ' + markdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '">' + repstr.replace(/(https:\/\/|http:\/\/|mailto:|ftp:\/\/)/gmi, '') + '</a>');
    }
    while ((stra = markdown.regexobject.reflinks.exec(str)) !== null) {
      helper1 = new RegExp('\\[' + stra[2] + '\\]: ?([^ \n]+)', "gi");
      if ((helper = helper1.exec(str)) !== null) {
        str = str.replace(stra[0], '<a ' + markdown.mmdCSSclass(helper[1], strict) + 'href="' + helper[1] + '">' + stra[1] + '</a>').replace(/^\s+|\s+$/g, '');
    ...
    while ((stra = markdown.regexobject.smlinks.exec(str)) !== null) {
      switch (stra[2]) {
      case 't':
      ...
      str = str.replace(stra[0], '<a ' + markdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '">' + stra[1] + '</a>');
    }
    while ((stra = markdown.regexobject.url2.exec(str)) !== null) {
      repstr = stra[1];
      str = str.replace(stra[0], '<a ' + markdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '">' + repstr + '</a>');
    }
    ...
    /* include */
    if ((markdown.useajax !== false) && (strict !== true)) {
    ...
    for(var index in markdown.codeblocks) { 
      if(markdown.codeblocks.hasOwnProperty(index)) {
        str = str.replace('§§§' + index + '§§§', markdown.codeblocks[index]); 
      }
    }
    str = str.replace('&#x0024&amp;', '$&');
