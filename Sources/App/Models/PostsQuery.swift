...
class Posts: DataQuery {
	...
	func getLatest(start: Int? = 0, limit: Int? = 30) -> Node? {
		let sql  = "Select p.*, f.name, f.dirname From posts p, forums f Where f.forumid = p.forumid And p.hidden = false Order by date Desc Offset $1 Limit $2"
	...
	func getLatest(forumId: Int, start: Int? = 0, limit: Int? = 30) -> Node? {
		let sql  = "Select p.*, f.name, f.dirname From posts p, forums f Where p.forumid = $1 And f.forumid = p.forumid And p.hidden = false Order by date Desc Offset $2 Limit $3"
