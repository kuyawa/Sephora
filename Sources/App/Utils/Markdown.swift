...
- Linux compatibility:
  Uses a Typealias defined in StringUtils.swift
  Uses an extension to TextCheckingResult defined in StringUtils.swift
...
private struct MarkdownRegex {
    ...
    fileprivate func replace(_ input: String, _ replacement: String) -> String {
        ...
        let result = regularExpression.stringByReplacingMatches(in: s as String,
    ...
    fileprivate func isMatch(_ input: String) -> Bool {
        let s = input as NSString
        let firstMatchRange = regularExpression.rangeOfFirstMatch(in: s as String,
            options: NSRegularExpression.MatchingOptions(rawValue: 0),
            range: NSMakeRange(0, s.length))
